...
class TemplateLocator extends BaseTemplateLocator implements FileLocatorInterface
{
    ...
    public function locate($template, $currentPath = null, $first = true)
    {
        ...
        $path = $this->rootDir .'/../src/'.$template->get('bundle')
            . '/Resources/views/';
        ...
        $isMob = $mobDetect->isMobile() && !$mobDetect->isTablet();
        $isTab = $mobDetect->isTablet();// && $mobDetect->isTablet();

        //echo ($isMob ? "MOB" : 'PAS MOB').'----'.($isTab ? "TAB" : 'PAS TAB');
        if ($isMob) {
            $mobPath = $path . '.mob.' . $template->get('format') . '.' . $template->get('engine');
            //echo $mobPath."<br />";
            //echo (file_exists($mobPath) ? "OUI" : "non")."<br />";
